...
##############################
# Create ISO Using LiveCD
##############################
pushd $SCRIPTS_DIRECTORY/../resources > /dev/null 2>&1
echo "Creating $BUILD_TYPE ISO."
time livecd-creator --config="$PATCHED_KICKSTART" --fslabel="$BUILD_SHORT" --cache="$CACHE_DIRECTORY"
if [ ! -f $TEMP_ISO ]; then
	echo "Couldn't create ISO."
	exit -1
fi
...
echo "Mounting generated ISO file."
...
##############################
# Create Temporary Directory and Build LiveCD
##############################
...
label linux
	...
	menu default
	kernel vmlinuz0
	append initrd=initrd0.img root=live:CDLABEL=$BUILD_SHORT rootfstype=auto liveimg biosdevname=0 ro xdriver=vesa nomodeset askmethod
label single
	menu label ^Single-User Mode
	kernel vmlinuz0
	append initrd=initrd0.img root=live:CDLABEL=$BUILD_SHORT rootfstype=auto liveimg biosdevname=0 ro single xdriver=vesa nomodeset askmethod
label install
	menu label ^Install
	kernel vmlinuz0
	append initrd=initrd0.img root=live:CDLABEL=$BUILD_SHORT rootfstype=auto liveimg biosdevname=0 liveinst ro xdriver=vesa nomodeset askmethod
label memtest
